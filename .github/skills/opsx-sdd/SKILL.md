---
name: opsx-sdd
description: 'OpenSpec OPSX 變更生命週期管理。使用 /opsx-explore 探索需求、/opsx-propose 建立提案、/opsx-apply 實作任務、/opsx-verify 驗證實作、/opsx-archive 歸檔變更。適用於任何專案的 spec-driven 開發流程。Use when: user types opsx-explore, opsx-propose, opsx-apply, opsx-verify, opsx-archive, or wants structured change management.'
metadata:
  author: opsx-sdd
  version: "1.0"
---
...
`/opsx-propose` 會在**主工作目錄**下直接操作 `openspec/` 結構，
所有後續指令都共用此結構：
...
`/opsx-propose` 在主工作目錄下建立專案目錄，所有 artifacts 位於其中的 `openspec/` 下：
