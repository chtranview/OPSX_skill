...
> 路徑均相對於專案根目錄 `<workspace>/<name>/`。本指令涵蓋 OpenSpec 的 sync + archive（無獨立 `/opsx-sync`）。
...
在主工作目錄下尋找專案目錄，讀取 `<workspace>/<name>/openspec/changes/` 目錄（排除 `archive/`），列出所有活躍 changes。
...
讀取 `<workspace>/<name>/openspec/changes/<name>/.openspec.yaml`，檢查所有 artifacts 狀態。
...
讀取 `<workspace>/<name>/openspec/changes/<name>/tasks.md`，統計：
- `- [ ]`（未完成）vs `- [x]`（已完成）
- 計算 complete / total
...
檢查 `<workspace>/<name>/openspec/changes/<name>/specs/` 目錄是否存在。
...
比對每個 delta spec 與對應的主 spec（`<workspace>/<name>/openspec/specs/<capability>/spec.md`）：
...
**目標檔案**：`<workspace>/<name>/openspec/specs/<capability>/spec.md`
- 若主 spec 不存在，建立新檔案
- 若存在，依操作類型修改
...
建立 `<workspace>/<name>/openspec/changes/archive/` 目錄（若不存在）。
...
若 `<workspace>/<name>/openspec/changes/archive/{target_name}/` 已存在：
- **停止並報錯**
- 建議：重命名既有歸檔或使用不同日期

#### 5d. 移動至 archive

將整個 `<workspace>/<name>/openspec/changes/<name>/` 目錄移至 `<workspace>/<name>/openspec/changes/archive/{target_name}/`。
