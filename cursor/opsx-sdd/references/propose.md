...
依序產出 proposal.md（what & why）→ specs/（delta 行為規格）→ design.md（how）→ tasks.md（implementation steps），
完成後即可進入 `/opsx-apply` 開始實作。

> 所有路徑相對於專案根目錄 `<workspace>/<name>/`（`<name>` 為 kebab-case 專案名稱，通常與 change 名稱相同）。
...
檢查主工作目錄下是否已有同名變更目錄 `<workspace>/<name>/openspec/changes/<name>/`。
...
**Step 3a. 建立專案根目錄（若尚未存在）**：
若 `<workspace>/<name>/` 不存在，則在主工作目錄下建立專案根目錄。

**Step 3b. 初始化 openspec 結構（若尚未存在）**：
若 `<workspace>/<name>/openspec/` 不存在，則在專案根目錄下建立：
```
...
建立於 `<workspace>/<name>/openspec/changes/<name>/proposal.md`。
...
**前置**：先讀取 `<workspace>/<name>/openspec/changes/<name>/proposal.md` 作為背景。

**此步驟為必要步驟。** 每個 change 都必須建立 delta specs，定義系統「應該做什麼」的結構化行為規格。

根據 proposal.md 的 Scope，識別需要規格化的 capability 領域，
在 `<workspace>/<name>/openspec/changes/<name>/specs/<capability>/spec.md` 建立。
...
**前置**：先讀取 `<workspace>/<name>/openspec/changes/<name>/proposal.md` 和 delta specs 作為背景。

建立於 `<workspace>/<name>/openspec/changes/<name>/design.md`。
...
**前置**：先讀取 `<workspace>/<name>/openspec/changes/<name>/proposal.md`、delta specs 和 `design.md` 作為背景。

建立於 `<workspace>/<name>/openspec/changes/<name>/tasks.md`。
...
確認以下檔案都存在：
- `<workspace>/<name>/openspec/changes/<name>/.openspec.yaml`（所有 artifacts 狀態為 `done`）
- `<workspace>/<name>/openspec/changes/<name>/proposal.md`
- `<workspace>/<name>/openspec/changes/<name>/specs/`（至少一個 `<capability>/spec.md`）
- `<workspace>/<name>/openspec/changes/<name>/design.md`
- `<workspace>/<name>/openspec/changes/<name>/tasks.md`
