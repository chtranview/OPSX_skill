...
這份文件說明 **opsx-sdd** 全域 Agent Skill 的功能摘要、檔案結構、以及五個 `/opsx-*` 指令的用法。適用於所有工作區中的 Antigravity Agent。
...
│           ├── _index.yaml                  ← 指令路由索引
│           ├── opsx-explore.yaml            ← 探索結構化定義
│           ├── opsx-propose.yaml            ← 提案結構化定義
│           ├── opsx-apply.yaml              ← 實作結構化定義
│           ├── opsx-verify.yaml             ← 驗證結構化定義
│           └── opsx-archive.yaml            ← 歸檔結構化定義
│
└── workflows/                               ← Slash Command 入口
...
| `workflows/*.md` | Slash Command | 註冊為 `/opsx-*` 指令，連結 Skill + 指引 |
...
- Antigravity Agent 開發環境
- 本 Skill 已放在 `.agent/skills/opsx-sdd/` 下
- Workflow 檔案已放在 `.agent/workflows/` 下

> 💡 Antigravity Agent 會自動載入 `.agent/skills/` 和 `.agent/workflows/`，無需額外設定。

### Step 1：開啟 Antigravity Chat

開啟 Antigravity Agent 面板。
...
**用途：** Skill 本身自動註冊的 slash command。當你不確定該用哪個 `/opsx-*` 指令時，輸入 `/opsx-sdd` 即可。
...
> 💡 此指令由 Antigravity 自動從 `SKILL.md` 的 `name: opsx-sdd` 註冊。五個 `/opsx-*` 指令則由 `.agent/workflows/` 中的 markdown 檔案註冊。
...
所有 artifacts 統一存放於主工作目錄的 `openspec/` 結構下：
...
**確認：**
1. `.agent/workflows/opsx-*.md` 檔案存在
2. `.agent/skills/opsx-sdd/SKILL.md` 的 `name` 欄位為 `opsx-sdd`
3. 重新啟動 Antigravity Agent 以重新載入設定。
...
### v1.0 調整為符合 Antigravity 與 OpenSpec 規範

為了符合 Antigravity 指令語法並維持檔案架構的一致性，我們對原先版本進行了以下修正：

- **環境與語境切換**：將原先針對 GitHub Copilot Chat 與 VS Code 的說明與路徑，全面替換為 Antigravity Agent。
- **檔案路徑與架構更新**：將 `.github/skills/` 與 `.github/prompts/` 的路徑描述修改為 `.agent/skills/` 與 `.agent/workflows/`。
- **Slash Commands 支援**：在 `.agent/workflows/` 目錄中建立了五個對應的 Markdown 執行腳本 (`opsx-explore.md` 等)，以無縫支援 Antigravity 的 `/` Slash 指令觸發機制。
