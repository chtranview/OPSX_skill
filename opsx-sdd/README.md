...
這份文件說明 **opsx-sdd** 全域 Agent Skill 的功能摘要、檔案結構、以及五個 `/opsx-*` 指令的用法。適用於所有工作區中的 VS Code Copilot Chat。
...
└── prompts/                                 ← Slash Command 入口
    ...
    ├── opsx-verify.prompt.md                ← /opsx-verify
...
| `prompts/*.prompt.md` | Slash Command | 註冊為 `/opsx-*` 指令，連結 Skill + 指引 |
...
> 💡 此指令由 VS Code 自動從 `SKILL.md` 的 `name: opsx-sdd` 註冊，無法移除。五個 `/opsx-*` 指令則由 `.github/prompts/` 中的 prompt 檔案註冊。
...
**確認：**
1. `.github/prompts/opsx-*.prompt.md` 檔案存在
